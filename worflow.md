...
当然，这些繁琐工作应该用一条命令来实现。:heart_kissing:
...
遇到新的需求，比如一个新的feature或是一个bugfix时，就要创建一个新的分支。新分支的名字需要具体一些，但不能是master这种特殊的功能分支。
...
开发完成，合并当前分支。如果该分支是bugfix类型的，那这个分支也可能没什么用了，直接删掉的说；feature类型的分支，可以在基础上继续接着开发。
